...
func (m Model) helpOverlay() string {
	rows := [][2]string{
		...
		{"~Text prompts (start / amend / move / note / add / due / tags)", ""},
		{"← / → · Home/End", "move the caret · Ctrl+A / Ctrl+E also jump to ends"},
		{"Ctrl+← / Ctrl+→", "jump the caret a word at a time"},
		{"⌫ / Del", "delete back / forward · Ctrl+W or Ctrl+⌫ deletes a word"},
		{"( [ { \" ' `", "auto-insert the closing pair (caret lands inside)"},
		{"⇥", "autocomplete a known project (start / move / add) or tag (#)"},
